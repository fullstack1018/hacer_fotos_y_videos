package com.makeedit.hacerfotosyvideoseditar.service;
...
import com.makeedit.hacerfotosyvideoseditar.OnProgressReceiver;
import com.makeedit.hacerfotosyvideoseditar.R;
import com.makeedit.hacerfotosyvideoseditar.activity.PlayVideoFromMyCreationActivity;
import com.makeedit.hacerfotosyvideoseditar.libffmpeg.FileUtils;
import com.makeedit.hacerfotosyvideoseditar.libffmpeg.Util;
import com.makeedit.hacerfotosyvideoseditar.util.ScalingUtilities;
...
public class CreateVideoService extends IntentService {
    ...
    private void createVideo() {
        ...
        try {
            process = Runtime.getRuntime().exec("chmod 777 "+inputCode);
...
//            Runtime.getRuntime().exec("chmod -R 777 " + ffmpeg.getAbsolutePath()).waitFor();
            process = Runtime.getRuntime().exec("chmod 777 "+new String[]{FileUtils.getFFmpeg(this), "-f", "concat", "-safe", "0", "-i", this.audioIp.getAbsolutePath(), "-c", "copy", "-preset", "ultrafast", "-ac", "2", this.audioFile.getAbsolutePath()});
            while (!Util.isProcessCompleted(process)) {
