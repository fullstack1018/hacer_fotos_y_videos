package com.makeedit.hacerfotosyvideoseditar.util;
...
public class ScalingUtilities {
    ...
    public static void convertPNGtoJPG(String pngFilePath, String jpgFilePath) {
        try {
            ...
            // Read the orientation information from the PNG file
            ExifInterface exif = new ExifInterface(pngFilePath);
            ...
            // Optionally, you can delete the original PNG file
    ...
    public static String convertPNGtoJPEG(File pngFile, File jpgFile) {

        Bitmap bitmap = BitmapFactory.decodeFile(pngFile.getAbsolutePath()); // Read the PNG image from the input path

        // Create a new blank bitmap with JPEG format
        Bitmap jpegBitmap = Bitmap.createBitmap(bitmap.getWidth(), bitmap.getHeight(), Bitmap.Config.RGB_565);
        Canvas canvas = new Canvas(jpegBitmap);
        canvas.drawBitmap(bitmap, 0, 0, null);

        try {
            jpegBitmap.compress(Bitmap.CompressFormat.JPEG, 95, new FileOutputStream(jpgFile));
...
//            jpegBitmap.compress(Bitmap.CompressFormat.JPEG, 100, outStream); // Adjust JPEG compression quality as needed
//            outStream.flush();
//            outStream.close();
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
    ...
    public static Bitmap checkBitmap(String path) {
        ...
        String outputPath;
        String num = xxx[xxx.length-1].substring(0,xxx[xxx.length-1].length()-3);
        outputPath = temp+"/"+num+"jpg";
        File inputFile = new File(path);
        File outputFile = new File(outputPath);

//        path = convertPNGtoJPEG(path, path.substring(0, path.length()-3)+"jpg");
        convertPNGtoJPG(inputFile.getAbsolutePath(), outputFile.getAbsolutePath());

        int orientation = 1;
        Options bounds = new Options();
        bounds.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(path, bounds);
        bounds.inPreferredConfig = Bitmap.Config.ALPHA_8;
        Bitmap bm = BitmapFactory.decodeFile(path, new Options());

        try {

            String orientString = new ExifInterface(path).getAttribute(ExifInterface.TAG_ORIENTATION);
